...
**Source:** [`vector-signed.json`](../../test-vectors/vector-signed.json)
...
| `citations` | 2 documents across 2 domains |
| `fragilityScore` | 0.5 |
...
```json
{
  "answerId": "b0000002-0001-4000-8000-000000000001",
  "citations": [
    {"id": "doc-test-policy-001", "quoteHash": "blake3:aaa..."},
    {"id": "doc-test-regulation-002", "quoteHash": "blake3:bbb..."}
  ],
  "counterfactual": null,
...
| `fragilityScore` | `fragility-score` | float |
...
| Field | Value | Purpose |
|-------|-------|---------|
| `snap-id` | `a0000002-...` | Receipt identifier for CWT subject binding |
| `tenant-id` | `tenant` | Issuer-scoped isolation |
| `receipt-hash` | `blake3:6024a5b2...` | Cross-reference to chain integrity hash |

**Output:** [`receipt-payload.cbor`](../cose-example/receipt-payload.cbor) (1,048 bytes)
...
Sig_structure = [
  ...
  payload_bytes           // CBOR receipt payload (1,048 bytes)
]
```
...
COSE_Sign1 = [
  ...
  payload:     1,048 bytes (CBOR receipt, kebab-case keys)
  signature:   64 bytes (ed25519)
]
```

**Output:** [`signed-statement.cbor`](../cose-example/signed-statement.cbor) (1,269 bytes)
...
The request body is the raw bytes of `signed-statement.cbor` (1,269 bytes). See [`scrapi-request.http`](scrapi-request.http) for the complete template.
...
```json
{
  ...
  "status": "succeeded",
  "entryInformation": {
    "signedStatementTimestamp": "2026-03-23T10:00:00.000Z",
    "issuer": "https://engine.cuecrux.com",
    "subject": "urn:crown:receipt:a0000002-0001-4000-8000-000000000001",
    "registrationTimestamp": "2026-03-23T10:00:01.234Z"
  }
}
```
...
PASS  signed-statement.cbor  [cose:valid]
  ...
  receipt-hash: blake3:6024a5b2f2e6935a37248bf2d162e1dd57975530db915f61cacc27ca7fee63d9
...
```bash
crown-verify vector-signed.json
```
