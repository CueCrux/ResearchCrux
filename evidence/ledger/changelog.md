...
## Phase 7.4: LLM Metadata Binding (2026-03-24)

### Schema 1.1 -- receipt-level LLM provenance

Two new hash-bound fields added to the CROWN receipt canonical payload:

- `llmModel` -- actual model string returned by the LLM API (e.g. `gpt-4o-mini-2024-07-18`)
- `llmRequestId` -- provider response/request ID for correlation

**Schema distinction:** `undefined` = schema 1.0 receipt (field absent from hash), `null` = schema 1.1 where LLM was not called (field included in hash as null). This preserves backward compatibility -- old receipts verify unchanged.

### 12/12 x 5 production server validation

| Run ID | Cat 2 citation_recall | Cat 7 broad | Cat 8 P@1 | Cat 11 broad | Cat 11 mdp | Cat 12 p/c | Result |
|---|---|---|---|---|---|---|---|
| `037b303a` | 0.678 | 1.000 | 0.963 | 0.927 | 1.000 | 1.000 | 12/12 |
| `80434381` | 0.633 | 1.000 | 0.963 | 0.927 | 1.000 | 1.000 | 12/12 |
| `69341abe` | 0.644 | 1.000 | 0.963 | 0.927 | 1.000 | 1.000 | 12/12 |
| `e0bfbd9b` | 0.670 | 1.000 | 0.963 | 0.927 | 1.000 | 1.000 | 12/12 |
| `fabf5dc8` | 0.693 | 1.000 | 0.963 | 0.927 | 1.000 | 1.000 | 12/12 |

Cat 7, 8, 11 (broad_recall + multi_doc_precision), 12 are perfectly deterministic across all 5 runs. Cat 2 citation_recall varies 0.633-0.693 (LLM-contingent). Cat 5 receipt chain 10/10 intact across all 5 runs -- schema 1.1 receipts chain correctly.

### Deployment scope

Zero retrieval/ranking/ingestion code changed. Changes limited to: `services/llm/types.ts`, `services/llm/providers/*.ts`, `services/llm.ts`, `services/receipts.ts`, `routes/answers.ts`, migration 134. `RECEIPT_SCHEMA_VERSION` bumped from 1.0 to 1.1 in code.

### Model-drift sentinel

No sentinel trigger. All three sentinel categories (Cat 2, 11, 12) pass consistently across 5 runs. Cat 2 citation_recall range (0.633-0.693) overlaps with 7.3 baseline range (0.670-0.715).

### Protocol artifacts

- CDDL: `? llm-model: tstr / null` and `? llm-request-id: tstr / null` added to `common-claims`
- JSON Schema: optional `llmModel` and `llmRequestId` properties added
- Verify library: `buildCanonicalPayload()` conditionally includes fields (undefined vs null distinction)
- Test vector: `vector-llm-metadata.json` (schema 1.1, BLAKE3 hash verified)
